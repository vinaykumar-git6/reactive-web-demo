...
public class ItemControllerTest {
	...
	public void getAllItems_Approach3() {
		Flux<Item> fluxItem = webTestClient.get().uri(ItemConstants.ITEM_END_POINT_V1).exchange().expectStatus().isOk()
	...
	public void getOneItem() {
		webTestClient.get().uri(ItemConstants.ITEM_END_POINT_V1.concat("/{id}"), "ABC").exchange().expectStatus().isOk()
	...
	public void getNotFound() {
		webTestClient.get().uri(ItemConstants.ITEM_END_POINT_V1.concat("/{id}"), "ABCD").exchange().expectStatus()
				.isNotFound();
	}
	...
	public void createItem() {
		...
		Item item = new Item(null, "Sunny Leone", 2000.2);

		webTestClient.post().uri(ItemConstants.ITEM_END_POINT_V1).contentType(MediaType.APPLICATION_JSON_UTF8)
				.body(Mono.just(item), Item.class).exchange().expectStatus().isCreated().expectBody()
				.jsonPath("$.description").isEqualTo("Sunny Leone").jsonPath("$.price").isEqualTo(2000.2);
	}
	...
	public void deleteItem() {
		...
		webTestClient.delete().uri(ItemConstants.ITEM_END_POINT_V1.concat("/{id}"), "ABC")
				.accept(MediaType.APPLICATION_JSON_UTF8).exchange().expectStatus().isOk().expectBody(Void.class);
	...
	public void updateItem_nonExistentId() {
		...
		double newPrice = 129.99;
		Item item = new Item(null, "BOSE TV", newPrice);
		webTestClient.put().uri(ItemConstants.ITEM_END_POINT_V1.concat("/{id}"), "ABCD")
				.contentType(MediaType.APPLICATION_JSON_UTF8).accept(MediaType.APPLICATION_JSON_UTF8)
